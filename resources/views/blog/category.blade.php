...
<head>
    ...
    <title>{{ $category->name }} — GoERP Blog</title>
    <meta name="description" content="Artikel kategori {{ $category->name }} di GoERP Blog.">
...
<main class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
    <div class="mb-10">
        <p class="text-sm text-stone-400 mb-1"><a href="/blog" class="text-indigo-600 hover:underline">Blog</a> / Kategori</p>
        <h1 class="text-3xl sm:text-4xl font-extrabold text-stone-900">{{ $category->name }}</h1>
        @if($category->description)
        <p class="text-stone-500 mt-2">{{ $category->description }}</p>
        @endif
    </div>
