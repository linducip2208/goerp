...
<head>
    <meta charset="UTF-8">
    <title>Faktur Penjualan {{ $invoice->invoice_no }}</title>
    <style>
        ...
        .invoice-title { font-size: 18px; font-weight: bold; color: #1e40af; text-align: right; }
        .invoice-title-sub { font-size: 11px; color: #64748b; text-align: right; }
        ...
        .items-table th { background: #2563eb; color: #fff; padding: 8px 6px; font-size: 9px; text-transform: uppercase; letter-spacing: 0.5px; text-align: left; }
        ...
        .summary-table .grand-row td { border-top: 2px solid #1e40af; padding-top: 6px; font-size: 12px; }
...
<body>
        ...
        <table class="header-table">
            <tr>
                ...
                <td width="40%">
                    ...
                    <div class="invoice-title-sub">No. {{ $invoice->invoice_no }}</div>
                </td>
            </tr>
        </table>
    </div>
    ...
    <table class="info-table">
        ...
        <tr>
            ...
            <td class="label">NPWP Customer</td>
            <td class="value">: {{ $invoice->customer->npwp ?? '-' }}</td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td class="value">: {{ strtoupper($invoice->status) }}</td>
            <td class="label">Sales</td>
            <td class="value">: {{ $invoice->sales->name ?? '-' }}</td>
        </tr>
    </table>
    ...
    <table class="items-table">
        ...
        <tbody>
            ...
            @empty
            <tr>
                <td colspan="9" style="text-align:center; color:#94a3b8; padding:20px;">Tidak ada item</td>
            </tr>
            @endforelse
        </tbody>
    </table>
    ...
    <table class="summary-table">
        <tr>
            ...
            <td class="value" style="width:50%;">Rp {{ number_format($invoice->subtotal, 2, ',', '.') }}</td>
        </tr>
        <tr>
            <td class="label">Diskon</td>
            <td class="value">Rp {{ number_format($invoice->discount_total, 2, ',', '.') }}</td>
        </tr>
        <tr>
            <td class="label">Pajak</td>
            <td class="value">Rp {{ number_format($invoice->tax_total, 2, ',', '.') }}</td>
        </tr>
        <tr class="grand-row">
            <td class="label" style="font-weight:bold; color:#1e40af;">Grand Total</td>
            <td class="value" style="color:#1e40af;">Rp {{ number_format($invoice->grand_total, 2, ',', '.') }}</td>
        </tr>
    </table>

    <div class="terbilang">
        Terbilang: {{ \App\Services\ReportPdfService::terbilang($invoice->grand_total) }} rupiah
    </div>

    <div style="display:flex; justify-content:flex-end; gap:80px; margin-bottom:30px;">
